...
import { conversacionesApi } from "@/lib/api-service"
import { Client } from "@stomp/stompjs"
// Eliminada la importación de SockJS
import { ChatSupport } from "./chat-support"
import { CustomerManagement } from "./customer-management"
import { WorkflowManagement } from "./workflow-management"
import { ApiConfigManagement } from "./api-config-management"
...
export function AdminDashboard() {
  ...
  // WebSocket con cliente nativo (sin SockJS)
  useEffect(() => {
    const wsUrlBase = process.env.NEXT_PUBLIC_WS_URL || "ws://localhost:8080/api/ws-chat/websocket"
    const wsUrl = wsUrlBase.replace(/^http/, 'ws')   // Convierte a ws:// o wss://

    const client = new Client({
      brokerURL: wsUrl,           // WebSocket nativo
      reconnectDelay: 5000,
      heartbeatIncoming: 4000,
      heartbeatOutgoing: 4000,
      onConnect: () => {
        // Suscripción global a cambios en conversaciones
        client.subscribe("/topic/conversaciones", () => {
          fetchUnreadCount()
          fetchConversationIds()
        })

        // Suscripciones individuales a cada conversación activa
        conversationIds.forEach((convId) => {
          if (!messageSubscriptionsRef.current.has(convId)) {
            const sub = client.subscribe(`/topic/conversacion/${convId}/mensajes`, () => {
              fetchUnreadCount()
      ...
      onDisconnect: () => {
        console.log("[AdminDashboard] WebSocket desconectado")
      },
      onStompError: (frame) => {
        console.error("[AdminDashboard] Error STOMP:", frame.headers["message"])
      },
    })
...
/*
  ...
  // WebSocket con cliente nativo y autenticación
  useEffect(() => {
    const token = getAuthToken()
    
    // Solo conectar si hay token
    if (!token) {
      console.log("[AdminDashboard] No hay token, esperando autenticación...")
      return
    }

    const wsUrlBase = process.env.NEXT_PUBLIC_WS_URL || "http://localhost:8080/api/ws-chat/websocket"
    const wsUrl = wsUrlBase.replace(/^http/, 'ws')

    console.log("[AdminDashboard] Conectando WebSocket con autenticación...")
    ...
    const client = new Client({
      ...
      heartbeatOutgoing: 4000,
      onConnect: () => {
        console.log("[AdminDashboard] WebSocket conectado exitosamente")
        
        // Suscripción global a cambios en conversaciones
        client.subscribe("/topic/conversaciones", () => {
          console.log("[AdminDashboard] Actualización de conversaciones recibida")
          fetchUnreadCount()
          fetchConversationIds()
        })
        ...
        conversationIds.forEach((convId) => {
          if (!messageSubscriptionsRef.current.has(convId)) {
            console.log(`[AdminDashboard] Suscribiendo a conversación ${convId}`)
            const sub = client.subscribe(`/topic/conversacion/${convId}/mensajes`, () => {
              console.log(`[AdminDashboard] Nuevo mensaje en conversación ${convId}`)
              fetchUnreadCount()
              if (incrementBadgeFnRef.current) {
                incrementBadgeFnRef.current(convId)
              }
      ...
      onWebSocketError: (event) => {
        console.error("[AdminDashboard] Error WebSocket:", event)
      }
    })
    ...
    return () => {
      console.log("[AdminDashboard] Limpiando suscripciones WebSocket")
      messageSubscriptionsRef.current.forEach((sub) => sub.unsubscribe())
      messageSubscriptionsRef.current.clear()
      if (stompClientRef.current?.connected) {
        stompClientRef.current.deactivate()
